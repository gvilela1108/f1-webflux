...
public interface ResultMapper {
    ...
    default Result fromClientToEntity(ResultsResponse resultsResponse, String round, Integer season) {
        return Result.builder()
                ...
                .millis(isEmpty(resultsResponse.getTime())?0:isEmpty(resultsResponse.getTime().getMillis())?0:Integer.valueOf(resultsResponse.getTime().getMillis()))
                .number(resultsResponse.getNumber())
                .points(isEmpty(resultsResponse.getPoints())?0:Integer.valueOf(resultsResponse.getPoints()))
                .position(resultsResponse.getPosition())
                .positionText(resultsResponse.getPositionText())
                .round(round)
                .season(season)
                .status(resultsResponse.getStatus())
                .time(isEmpty(resultsResponse.getTime())?"A":isEmpty(resultsResponse.getTime().getTime())?"0":resultsResponse.getTime().getTime())
                .build();
    }


    default ResultsResponse fromEntityToResponse(Result result, DriverResponse driver, ConstructorResponse constructor, FastestLapResponse fastestLap) {
        var time = Time.builder().
                time(result.getTime())
                .millis(String.valueOf(result.getMillis()))
                .build();
