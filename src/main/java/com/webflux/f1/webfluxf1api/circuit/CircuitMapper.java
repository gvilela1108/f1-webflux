...
public interface CircuitMapper {
    ...
    default Circuit fromClientToEntity(CircuitResponse circuit) {
        return Circuit.builder()
                ...
                .latitude(isEmpty(circuit.getLocation().getLatitude())?0:Float.valueOf(circuit.getLocation().getLatitude()))
                .locality(circuit.getLocation().getLocality())
                .longitude(isEmpty(circuit.getLocation().getLongitude())?0:Float.valueOf(circuit.getLocation().getLongitude()))
                .url(circuit.getUrl())
                .build();
    }
