...
public class RaceServiceImpl implements RaceService {
  ...
  public Mono<RaceResponse> getRaceResult(int season, String round) {
    ...
    if (season > LocalDate.now().getYear()) {
      log.debug(
          "Temporada superior ao ano corrente m=getRaceResults season={} round={}", season, round);
  ...
  public void saveRaceResult(ArrayList<Races> races, String round, Integer season) {
    ...
    try {
      ...
      if (isEmpty(raceExists)) {
        ...
        var circuitExists =
            circuitRepository.findByCircuitId(
                races.get(NumberUtils.INTEGER_ZERO).getCircuit().getCircuitId());
        if (isEmpty(circuitExists))
          circuitRepository.save(
              circuitMapper.fromClientToEntity(races.get(NumberUtils.INTEGER_ZERO).getCircuit()));
                      ...
                      .forEach(
                          resultRace -> {
                            var constructorExists =
                                constructorRepository.findByConstructorId(
                                    resultRace.getConstructor().getConstructorId());
                            if (isEmpty(constructorExists))
                              constructorRepository.save(
                                  constructorMapper.fromClientToEntity(
                                      resultRace.getConstructor()));

                            var driverExists =
                                driverRepository.findByDriverId(
      ...
      log.error(
          "Erro ao salvar na base de dados m=saveRaceResult season={} round={} e={}",
          season,
          round,
          e.getStackTrace());
    ...
    races.stream()
        .forEach(
            race -> {
              ...
              var results =
                  resultRepository.findBySeasonAndRound(race.getSeason(), race.getRound());
              results.stream()
                  .forEach(
                      result -> {
              ...
              racesResponse.add(
                  raceMapper.fromEntityToResponse(
                      race, circuitMapper.fromEntityToResponse(circuit), resultsResponses));
            });
